...
class HamsterKombatApp:
    ...
    def create_table(self):
        ...
        self.tree = ttk.Treeview(self.table_frame, columns=("category", "card_name", "value"), show="headings")
    ...
    def get_data(self, category):
        ...
        tk.Label(data_window, text="Coin Value:").pack(pady=5)
        ...
        def submit_data():
            ...
            self.entries.append((category, card_name, value))
            self.save_data()
            self.update_table()
            data_window.destroy()
    ...
    def on_item_double_click(self, event):
        ...
        category, card_name, value = item['values']
        ...
        tk.Label(edit_window, text="Coin Value:").pack(pady=5)
        ...
        tk.Label(edit_window, text="Value:").pack(pady=5)
        value_entry = tk.Entry(edit_window)
        value_entry.insert(0, value)
        value_entry.pack(pady=5)

        def calculate_value(event=None):
            if coin_entry.get() and profit_entry.get():
                try:
                    coin = float(coin_entry.get())
                    profit = float(profit_entry.get())
                    calculated_value = coin / profit
                    value_entry.delete(0, tk.END)
                    value_entry.insert(0, calculated_value)
                except ValueError:
                    pass

        coin_entry.bind("<KeyRelease>", calculate_value)
        profit_entry.bind("<KeyRelease>", calculate_value)
        ...
        def submit_edit():
            ...
            if coin_entry.get() and profit_entry.get():
                try:
                    coin = float(coin_entry.get())
                    profit = float(profit_entry.get())
                    new_value = coin / profit
                except ValueError:
                    pass

            for i, entry in enumerate(self.entries):
                if entry[1] == card_name:  # Find by card name
                    self.entries[i] = (new_category, new_card_name, float(new_value))
                    break
    ...
    def update_table(self):
        ...
        for i, (category, card_name, value) in enumerate(sorted_entries):
            self.tree.insert("", "end", values=(category, card_name, value))
    ...
    def load_data(self):
        if os.path.exists("combats.txt"):
            with open("combats.txt", "r") as f:
                for line in f:
                    category, card_name, value = line.strip().split(",")
                    self.entries.append((category, card_name, float(value)))
    ...
    def update_table_with_filtered_data(self, filtered_entries):
        ...
        sorted_entries = sorted(filtered_entries, key=lambda x: x[2])
        for i, (category, card_name, value) in enumerate(sorted_entries):
            self.tree.insert("", "end", values=(category, card_name, value))
